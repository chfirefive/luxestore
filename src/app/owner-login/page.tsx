  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const storedPassword = await getOwnerPassword();
        console.log('Stored password retrieved:', storedPassword);
      ...
      <div className="glass-panel" style={{ width: '100%', maxWidth: '420px', padding: '3rem 2.5rem', textAlign: 'center' }}>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', textAlign: 'left' }}>
          ...
          <button type="submit" className="btn-primary" disabled={isAuthenticating}
            style={{ padding: '14px', fontSize: '1rem', opacity: isAuthenticating ? 0.75 : 1, cursor: isAuthenticating ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
            {isAuthenticating
